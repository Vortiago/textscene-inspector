/**
 * Semantic linter rules for StaticBody2D
 *
 * Note: Format validation (collision_layer values, Vector2 format, etc.) is handled
...
/**
 * Vector2 format regex for extracting numeric values
 */
const VECTOR2_REGEX = /^Vector2\(\s*(-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*,\s*(-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*\)$/;

/**
 * Check if a Vector2 string is non-zero (any component != 0)
 */
function isNonZeroVector2(value: string): boolean {
...
/**
 * Validate StaticBody2D semantic rules
 */
function checkStaticBody2D(context: RuleContext): Diagnostic[] {
  const diagnostics: Diagnostic[] = [];
  const { node, scene } = context;

  // Only run for StaticBody2D nodes
  ...
  // Warning: StaticBody2D without CollisionShape2D children is useless
  if (!hasCollisionShapeChild(node)) {
    diagnostics.push({
      severity: 'warning',
      message: `StaticBody2D '${node.name}' has no CollisionShape2D children. Static bodies need collision shapes to function in physics.`,
      nodeName: node.name,
      nodeType: node.type,
  ...
  if (rawProps.constant_linear_velocity && isNonZeroVector2(rawProps.constant_linear_velocity)) {
    diagnostics.push({
      severity: 'warning',
      message: `StaticBody2D '${node.name}' has non-zero constant_linear_velocity (${rawProps.constant_linear_velocity}). This affects touching bodies but doesn't move the static body itself, which can be confusing.`,
      nodeName: node.name,
      nodeType: node.type,
  ...
  // In 2D, angular velocity is a float, not a vector
  if (rawProps.constant_angular_velocity) {
    ...
    if (!isNaN(angularVel) && angularVel !== 0) {
      diagnostics.push({
        severity: 'warning',
        message: `StaticBody2D '${node.name}' has non-zero constant_angular_velocity (${rawProps.constant_angular_velocity}). This affects touching bodies but doesn't rotate the static body itself, which can be confusing.`,
        nodeName: node.name,
        nodeType: node.type,
        ruleName: 'staticbody2d-constant-velocity-warning',
      });
  ...
  if (rawProps.collision_layer !== undefined) {
    ...
    if (!isNaN(collisionLayer) && collisionLayer === 0) {
      diagnostics.push({
        severity: 'warning',
        message: `StaticBody2D '${node.name}' has collision_layer set to 0. The body won't be on any collision layer and may not interact with other physics objects.`,
        nodeName: node.name,
        nodeType: node.type,
  ...
  if (rawProps.collision_mask !== undefined) {
    ...
    if (!isNaN(collisionMask) && collisionMask === 0) {
      diagnostics.push({
        severity: 'warning',
        message: `StaticBody2D '${node.name}' has collision_mask set to 0. The body won't collide with any layers and may not detect collisions.`,
        nodeName: node.name,
        nodeType: node.type,
...
/**
 * StaticBody2D semantic validation rule
 */
const staticBody2DValidationRule: LintRule = {
  meta: {
    name: 'valid-staticbody2d',
    description: 'Validates StaticBody2D resource references, collision shapes, and physics configuration',
    category: 'validation',
    applicableNodeTypes: ['StaticBody2D'],
  },
  check: checkStaticBody2D,
};
