...
describe('decodeEnvironment — AgX carries its own white', () => {
  ...
  it('reads an authored tonemap_agx_white', () => {
    expect(decodeEnvironment({ tonemap_agx_white: '6.5' }).tonemap_agx_white).toBeCloseTo(6.5, 6);
  });

  it('falls back to the default on garbage rather than storing NaN (error path)', () => {
    const r = decodeEnvironment({ tonemap_agx_white: 'bright' });
    expect(r.tonemap_agx_white).toBeCloseTo(16.29, 6);
  });
});
