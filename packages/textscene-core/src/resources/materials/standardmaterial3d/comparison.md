...
- **albedo_texture_force_srgb** — Godot's flag exists for a texture IMPORTED without an sRGB format: its sampler then reads stored bytes and the shader decodes them itself (`scene/resources/material.cpp:1659`). Every image reaching this renderer is decoded through the browser and tagged sRGB, so the albedo slot already decodes and the flag is a no-op here. A project that authored it on top of a genuinely non-sRGB import would read one decode short.
...
- **ao_light_affect** — `ao_texture` reaches three's `aoMap`, which attenuates INDIRECT
  light only. Godot multiplies the DIRECT term by it too, scaled by this property:
  `ao = mix(1.0, ao, ao_light_affect)` in `scene_forward_clustered.glsl`, fed from
  `BaseMaterial3D::_update_shader` (`scene/resources/material.cpp`). The property is
  not read anywhere here, so at its maximum Godot draws a hard checkerboard where
  this draws a nearly flat plate — around 60/255 mean channel error over the pixels
  carrying the map. Reaching it needs the multiply injected into the lighting chain
  via `onBeforeCompile`, which splits the shared material per value.
