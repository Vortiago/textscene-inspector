...
describe('UV transform integration — shared textures', () => {
  ...
  it('handles six materials with different UV scales (showcase scenario)', () => {
    ...
    // Six clones, the identity scale included: a default material asks for
    // Repeat and the loader hands a clamp entry, so wrapping diverges for every
    // material whatever its UV transform.
