/**
 * Curve2D parsing + polyline tessellation for Path2D / PathFollow2D.
 *
 ...
 *
 * Pure module (no THREE): coordinates stay in Godot 2D space (+Y down). The
 * Path2D component negates Y per vertex when it builds the gizmo geometry, and
 * PathFollow2D negates Y when it conjugates the follow transform — both matching
 * the Node2D `diag(1,-1,1)` convention (see node2dTransform / Polygon2D).
 */

import { parsePackedVector2Array } from './packedArray';
