...
describe('createSceneProcessor (WI-ARCH-2 replacement for SceneLoader)', () => {
  ...
  beforeEach(() => {
    ...
    processor = createSceneProcessor({
      ...
      resolveMetadata: (idOrPath) => {
        ...
        return m ? { path: m.path, type: m.type } : null;
      },
  ...
  describe('lenient parsing behavior', () => {
    // The TscnParser is designed to recover from errors rather than fail;
    // these tests confirm that lenient behavior survives the unification.

    it('returns empty scene for empty content (lenient)', async () => {
    ...
    it('handles invalid content gracefully (lenient)', async () => {
