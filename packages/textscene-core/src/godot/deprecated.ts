...
const DEPRECATED_PROPERTY_NAMES: Readonly<Record<string, Readonly<Record<string, string>>>> = {
...
export function canonicalPropertyName(nodeType: string | undefined, key: string): string {
  ...
  return nodeType === undefined ? key : (DEPRECATED_PROPERTY_NAMES[nodeType]?.[key] ?? key);
}
...
export function isDeprecatedPropertyName(nodeType: string | undefined, key: string): boolean {
  return nodeType !== undefined && DEPRECATED_PROPERTY_NAMES[nodeType]?.[key] !== undefined;
}
