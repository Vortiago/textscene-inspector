 ...
 * spelling: the linter and the renderer once each built the transform themselves.
 */
...
describe('the one spelling of the Transform2D construction and product', () => {
  ...
  it('detects both shapes before trusting their absence', () => {
  ...
  it('composes two transforms only here', () => {
    const composers = sources.filter(({ text }) => PRODUCT.test(text)).map(({ rel }) => rel);
