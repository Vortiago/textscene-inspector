/**
 * Bug archaeology: port original 401f8f5 assertions to verify they pass on current code.
 *
 * 401f8f5 ("Fix Transform3D decomposition for rotated+scaled planes", Nov 2025)
 * established that the ShortWall transform should decompose to scale.z=6, rotation.y=+π/2.
 *
 * b4ccaab ("WI-R3F-10", May 2026) re-broke decomposeTransform3D by treating basis vectors
 * as columns instead of rows, then rewrote these tests to assert the buggy output
 * (scale.x=6, rotation.y=-π/2) with a comment saying the user intent "isn't recoverable
 * from the flat serialisation". That was incorrect.
 *
 * 99c1479 ("fix(walls)", May 2026) corrected the row-vector convention again.
 *
 * This file ports the 401f8f5 original assertions directly against current code.
 * Tests that PASS confirm the bug is fixed. Tests that FAIL indicate latent bugs.
 */

import { describe, it, expect } from 'vitest';
import { parseTransform3D, decomposeTransform3D } from '../utils/transform';
import type { Transform3D } from '../nodes/base/node3d/types';

describe('archaeology: 401f8f5 original assertions vs current code', () => {
