/**
 * Archaeology: ported from main:packages/textscene-core/src/core/SceneManager.nested-external.test.ts
 *
 * Original tested 3-level nested scenes (TopScene → MiddleScene → LeafScene)
 * using the deleted SceneManager/NodeTracker/NodeLifecycleManager stack.
 *
 * R3F port: uses the same fake loader pattern from NodeDispatcher.instance.test.tsx.
 * The `InstancedSceneSubtree` in NodeDispatcher.tsx resolves ExtResource refs
 * recursively, so 3-level nesting should work transparently.
 *
 * Key assertions mapped:
 * - `nodeTracker.getAllPaths().toContain(path)` → find named group in R3F scene
 * - `nodeTracker.getObject(path).userData.instanceRoot` → DELETED_FEATURE (not set in R3F)
 * - `leafInstance.position.x ≈ 2` → find group by name, check position
 * - Level 3 nodes present → named meshes found in R3F tree
 *
 * Missing resource tests: when the middle scene is absent the loader returns null
 * for that path, InstancedSceneSubtree renders a placeholder, and level-3 nodes
 * should not appear in the tree.
 */
...
describe('SceneManager - Nested External Scenes 3+ Levels (ported to R3F)', () => {
