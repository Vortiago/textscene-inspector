/**
 * Archaeology: ported from main:packages/textscene-core/src/core/SceneManager.external-rendering.test.ts
 *
 * Original tested `SceneManager.addScene()`, `NodeTracker`, `NodeLifecycleManager`,
 * `ResourceRegistry` (all deleted). The R3F equivalent is:
 *   ResourceLoader (fake) + SceneResourcesProvider + NodeDispatcher
 *
 * Key differences:
 * - No `nodeTracker.getObject(path)` — R3F tree is queried via ReactThreeTestRenderer
 * - No `userData.instanceRoot` — R3F doesn't tag nodes this way; checked via tree structure
 * - No `instanceNode.parent === threeScene` — verified by finding named groups in the R3F tree
 * - Scene caching: `makeLoader()` stubs `loader.scenes.getCached` — if the same path is
 *   requested twice, the same cache entry is returned without another `requestScene` call
 *
 * The original test "should cache and reuse parsed external scenes" checked
 * `resourceRegistry.loadByPath` was called once. In R3F world the loader's `request` spy
 * serves the same role — it should only be called once for the same path even across
 * multiple `<InstancedSceneSubtree>` renders.
 *
 * NOTE: The `should set instanceRoot userData` tests have NO direct R3F equivalent
 * because `InstancedSceneSubtree` does not tag objects with `userData.instanceRoot`.
 * Those tests are marked DELETED_FEATURE with a comment explaining why.
 */
...
describe('SceneManager - External Scene Node Rendering (ported to R3F)', () => {
