 ...
 * - **Inherited modulate resets to white, and the texture sampler ambient
 *   resets to the viewport default.** `CanvasLayer` derives from `Node`, so
 *   `get_parent_item()` returns null under it (`scene/main/canvas_item.cpp:565`)
 *   — the child attaches to this layer's own canvas RID (`canvas_item.cpp:264,269`),
 *   every canvas seeds its root items white
 *   (`servers/rendering/renderer_canvas_cull.cpp:82`), and the SAME null
 *   `parent_item` is what `_refresh_texture_filter_cache`/`_refresh_texture_repeat_cache`
 *   (`canvas_item.cpp:1625-1699`) test before falling back to the root default
 *   rather than an ancestor's cache. Visibility crosses only because
 *   `scene/main/canvas_layer.cpp:57-63` propagates it by hand; neither colour
 *   nor the sampler ambient has an equivalent.
