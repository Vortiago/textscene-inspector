...
describe('<ControlFallback>', () => {
  it('draws exactly one LineSegments outline', async () => {
    const renderer = await ReactThreeTestRenderer.create(
      <ControlFallback solveNode={solveNode()} rect={{ x: 0, y: 0, w: 80, h: 24 }} />
    );
  ...
  it('sizes the outline geometry to the solved rect (w, h)', async () => {
    const renderer = await ReactThreeTestRenderer.create(
      <ControlFallback solveNode={solveNode()} rect={{ x: 0, y: 0, w: 64, h: 32 }} />
    );
    const line = renderer.scene.findByType('LineSegments');
    const geometry = line.instance.geometry;
  ...
  it('handles a zero-size rect without throwing (edge case: unsolved/degenerate rect)', async () => {
    await expect(
      ReactThreeTestRenderer.create(<ControlFallback solveNode={solveNode()} rect={{ x: 0, y: 0, w: 0, h: 0 }} />)
    ).resolves.toBeTruthy();
  });
});
