...
/**
 * CheckButton's eight icon draw states: the four plain (`default_theme.cpp:327-330`) and their RTL
 * twins (`:332-335`), separate SVGs rather than a flip. `CheckButton::_notification` selects the
 * twins whole under `is_layout_rtl()` (`check_button.cpp:109-120`).
 */
...
/**
 * FoldableContainer's four fold-state arrows (`default_theme.cpp:1329-1332`). `_get_title_icon`
 * (`foldable_container.cpp:428-435`) reaches `expanded_arrow_mirrored` through `title_position`,
 * and never picks the RTL-only `folded_arrow_mirrored` here.
 */
...
/**
 * CodeEdit's fold-gutter icons (`default_theme.cpp:496-499`). Only `can_fold` and
 * `can_fold_code_region` (`default_theme.cpp:496,498`) are vended: nothing in a `.tscn` folds a line.
 * `can_fold` is `arrow_down.svg`, the asset of FoldableContainer's `expanded_arrow`.
 */
...
/** The `draw_tabs` and `draw_spaces` glyphs, bound to the same keys and icons in TextEdit and CodeEdit (`default_theme.cpp:457-458,491-492`). */
...
/** ScrollContainer's edge fades, which `_update_scroll_hints` shows where content continues past an edge (`default_theme.cpp:667-668`). */
...
/**
 * `scene/theme/icons/color_picker_bar_arrow.svg` (16x16, `viewBox="0 0 16 20"`): `ColorPicker`'s
 * `bar_arrow` (`default_theme.cpp:1099`). It overrides the `grabber` of `sliders[0..2]` and
 * `alpha_slider` (`color_picker.cpp:636-637,646-647`); `intensity_slider` keeps the HSlider grabber.
 */
...
/** ColorPicker's button and bar icons (`default_theme.cpp:1088-1101`). `shape_circle` and `shape_rect_wheel` are not vended, since the previewer draws only `SHAPE_HSV_RECTANGLE`. */
