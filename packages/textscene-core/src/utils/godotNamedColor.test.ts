/**
 * `Color::named`/`Color::find_named_color` (`core/math/color.cpp:396-431`)
 * over the X11 table in `core/math/color_names.inc`. Every expectation below
 * is read straight off that table's own `Color::hex(0xRRGGBBAA)` literal —
 * never off a rendered pixel.
 */
...
describe('godotNamedColor', () => {
  ...
  it('carries an entry whose alpha is not 1', () => {
    // :184 — TRANSPARENT is white at alpha 0, so an implementation that only
    // stored RGB would resolve it to opaque white.
  ...
  it('returns undefined for a name the table does not carry, so the caller can apply its own fallback', () => {
    ...
    // (`color.cpp:404-410`); this reports the miss instead of choosing for the
    // caller, since `from_string`'s default differs per call site.
    expect(godotNamedColor('nosuchcolour')).toBeUndefined();
    expect(godotNamedColor('')).toBeUndefined();
    // A hex string is not a NAME — `Color::from_string` tries `html` first and
    // only falls through to here, so the table must not accidentally match one.
    expect(godotNamedColor('#ff0000')).toBeUndefined();
  });

  it('carries the whole table, not a convenient subset', () => {
    // `get_named_color_count()` — 146 entries in 4.6.3. A partial port is
    // exactly the failure this replaces: it looks correct for `red` and
    // silently falls back for the other 145.
