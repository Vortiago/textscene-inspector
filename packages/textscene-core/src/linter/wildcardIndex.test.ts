/**
 * The four wildcard key shapes. Routing is wider than what Godot accepts, so a
 * refused index still reaches the family's dispatcher: each matcher checks
 * structure only and leaves the integer to `indexParse`. A matcher that accepts
 * too much routes an unrelated key to a dispatcher, so the negatives matter too.
 */
...
describe('wildcard index', () => {
  ...
  /** The original two shapes' routing, unchanged by the subtree and terminal shapes. */
