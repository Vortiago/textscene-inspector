/**
 * DirectionalLight2D registration — parsed and validated, not yet rendered.
 *
...
describe('DirectionalLight2D registration', () => {
  ...
  it('registers no render component, so it still reads as not implemented', () => {
    expect(nodeComponentRegistry.get('DirectionalLight2D')).toBeUndefined();
  });
  ...
  it('lands in the lenient parser tree with its type preserved and no fallback warning', () => {
    ...
    const scene = new TscnParser().parse(
      '[gd_scene format=3]\n\n[node name="Root" type="Node2D"]\n\n' +
        '[node name="MyDirectionalLight2D" type="DirectionalLight2D" parent="."]\n'
    );

    const node = scene.nodes[0]?.children[0];
    expect(node?.type).toBe('DirectionalLight2D');
