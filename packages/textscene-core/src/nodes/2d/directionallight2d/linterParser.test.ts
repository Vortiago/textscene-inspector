/**
 * DirectionalLight2D strict validators — format and range checks.
 *
 ...
 * DirectionalLight2D binds, or set DECLARES_NOTHING when it binds no ADD_PROPERTY at all.
...
/**
 * Keys DirectionalLight2D does NOT declare, each paired with the ancestor that does.
 * Name at least one; Light2D is where to start.
 *
 ...
 * an EMPTY set and passes while asserting nothing — "Godot gives DirectionalLight2D no
...
describe('DirectionalLight2D strict validators', () => {
  ...
  it('rejects a malformed value on every property it validates', () => {
    ...
    // DirectionalLight2D declares nothing, which is what INHERITED below covers.
  ...
  it('resolves each inherited key to the ancestor that declares it', () => {
    ...
    for (const [owner, key] of INHERITED) {
      ...
      // DirectionalLight2D would answer here while drifting from the ancestor's rule.
