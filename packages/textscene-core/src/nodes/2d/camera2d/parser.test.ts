...
describe('parseCamera2D', () => {
  ...
  it('zoom accepts fractional values', () => {
    const p = parseCamera2D(heading('Camera2D', { name: 'Cam' }), { zoom: 'Vector2(0.5, 0.75)' });
    expect(p.zoom).toEqual({ x: 0.5, y: 0.75 });
  });

  it('zoom accepts zero values', () => {
    const p = parseCamera2D(heading('Camera2D', { name: 'Cam' }), { zoom: 'Vector2(0, 0)' });
    expect(p.zoom).toEqual({ x: 0, y: 0 });
  });

  it('zoom accepts negative values', () => {
    const p = parseCamera2D(heading('Camera2D', { name: 'Cam' }), { zoom: 'Vector2(-1, -2)' });
    expect(p.zoom).toEqual({ x: -1, y: -2 });
  });

  it('zoom with scientific notation', () => {
    const p = parseCamera2D(heading('Camera2D', { name: 'Cam' }), { zoom: 'Vector2(1e-3, 2.5e2)' });
  ...
  it('malformed zoom falls back to default (1, 1)', () => {
    ...
    const p = parseCamera2D(heading('Camera2D', { name: 'Cam' }), { zoom: 'not-a-vector' });
    expect(p.zoom).toEqual({ x: 1, y: 1 });
  });

  it('malformed offset falls back to default (0, 0)', () => {
    const p = parseCamera2D(heading('Camera2D', { name: 'Cam' }), { offset: 'Vector2(oops)' });
