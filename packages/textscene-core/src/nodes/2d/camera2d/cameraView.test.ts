...
import { camera2DView, type Camera2DFraming } from './cameraView';
import { Camera2DAnchorMode } from './types';

const VIEWPORT = { x: 1152, y: 648 };

function props(overrides: Partial<Camera2DFraming> = {}): Camera2DFraming {
  return {
    zoom: { x: 2, y: 2 },
    offset: { x: 0, y: 0 },
    anchor_mode: Camera2DAnchorMode.DRAG_CENTER,
    // Matches Camera2D's parser defaults (parser.ts): unlimited in every direction.
    limitLeft: -10000000,
    limitTop: -10000000,
    limitRight: 10000000,
