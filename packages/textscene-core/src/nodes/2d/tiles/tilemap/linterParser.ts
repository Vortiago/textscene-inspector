/**
 * TileMap strict validators. `layer_<i>/*` is a glued-index `PropertyListHelper`
 * family (tile_map.cpp:1023-1043), like TabBar's `tab_<i>/*`, of seven leaves plus
 * `tile_data`. See propertyListRouteCoverage.test.ts.
 */
...
/**
 * tile_map.h:56-59, VisibilityMode: 3 BIND_ENUM_CONSTANTs, the same shape and hint
 * string as TileMapLayer's DebugVisibilityMode (tilemaplayer/linterParser.ts).
 */
...
const LAYER_LEAVES: Readonly<Record<LayerLeaf, PropertyValidator>> = {
  ...
  // tile_map.cpp:1035, Variant::INT. set_layer_z_index forwards to CanvasItem's
  // own z_index setter, whose ERR_FAIL_COND against CANVAS_ITEM_Z_MIN/MAX
  // (canvas_item.cpp:668) is enforced.
...
const layerValidator = indexedFamilyValidator({
  ...
  // is a dropped write, as in TabBar's tab_<i>/* family.
...
validatorRegistry.registerAll('TileMap', {
  ...
  // internal format-version tag Godot always writes. See tileDataSlots.ts.
  ...
  // tile_map.cpp:223-231: ERR_FAIL_COND_MSG(p_size < 1) floors it, and each child
  // layer's setter re-enforces it. No setter checks the hint's 128 ceiling
  // (tile_map.cpp:996), so it warns. TileMapLayer's own rendering_quadrant_size
  // has no RANGE hint (tilemaplayer/linterParser.ts).
