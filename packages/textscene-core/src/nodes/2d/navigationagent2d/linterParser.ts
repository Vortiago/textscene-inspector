/**
 * NavigationAgent2D strict validators for linting: every `ADD_PROPERTY` in
 * `navigation_agent_2d.cpp:143-188`, none with `overrides=` in
 * doc/classes/NavigationAgent2D.xml. Its base is
 * plain `Node`, not Node2D: it steers a Node2D parent, which linter.ts checks.
 */
...
validatorRegistry.registerAll('NavigationAgent2D', {
  ...
  // bare assignment, so only the literal's shape is checked.
  ...
  // set_path_metadata_flags (cpp:524-530) is a bare BitField assignment with no
  // `p_flags & MASK`, so every bit is kept. A bit outside the HINT_FLAGS at :151
  // (1|2|4, navigation_constants_2d.h:52-54) is unreachable from the inspector:
  // `hintedBitField`'s warning tier.
  ...
  // alters an out-of-range value, so this is an error (ADR-0032), not a warning.
  ...
  // :157 has no hint string, so the bound comes only from the clamp.
  ...
  // at :161, still serialised, object.h:132), so only the literal's shape is checked.
  ...
  // `ERR_FAIL_COND_MSG(p_max_speed < 0.0, "Max speed must be positive.")` refuses
  // below 0, and the hint (:167, "0.01,100000,0.01,or_greater,suffix:px/s") floors
  // at 0.01, so [0, 0.01) loads and only warns. `or_greater` opens the ceiling.
  ...
  // `#ifdef DEBUG_ENABLED`, which every editor build defines), so only the
  // literal's shape is checked.
  ...
  // clamps: `debug_path_custom_point_size = MAX(0.0, p_point_size);` at :1050. The
  // editor, which saves a .tscn, always builds with DEBUG_ENABLED.
  ...
  // bare assignment with no clamp. The hint at :188 ("-1,50,0.01,or_greater") gives
  // an advisory floor of -1, the default sentinel for "use the global line width".
