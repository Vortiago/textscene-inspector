 ...
 * the point array, stroke geometry, and the gradient/texture/curve resource
 * slots that skin it.
 */
...
validatorRegistry.registerAll('Line2D', {
  ...
  // (line_2d.cpp:122-124) returns `Vector<Vector2>` directly — the packed type
  // itself, not a TypedArray getter — so `PackedVector2Array(...)` is the only
  ...
  // never setter-enforced, so it is a warning.
