/**
 * PointLight2D semantic rules — the range windows that can never match.
 *
...
describe('PointLight2D linter', () => {
  it('warns when the z window is inverted', () => {
    expectDiagnostic(
      scene(node('PointLight2D', { range_z_min: 5, range_z_max: 4 })),
      {
  ...
  it('warns when the layer window is inverted', () => {
    expectDiagnostic(
      scene(node('PointLight2D', { range_layer_min: 2, range_layer_max: 1 })),
      {
  ...
  it('never raises an error, so a fixture carrying one still lints clean', () => {
    const diagnostics = lint(
      scene(node('PointLight2D', { range_z_min: 5, range_z_max: 4, range_layer_min: 2, range_layer_max: 1 }))
    );
  ...
  it('accepts a single-value window, which is Godot\'s own layer default', () => {
    ...
    expectNoDiagnostic(
      scene(node('PointLight2D', { range_layer_min: 0, range_layer_max: 0, range_z_min: 3, range_z_max: 3 })),
      { ruleName: 'pointlight2d-inverted-layer-range' }
    );
    expectNoDiagnostic(
      scene(node('PointLight2D', { range_z_min: 3, range_z_max: 3 })),
      { ruleName: 'pointlight2d-inverted-z-range' }
    );
  });

  it('says nothing about a light that authors no window at all', () => {
    expect(lint(scene(node('PointLight2D')))).toEqual([]);
  });
  ...
  it('compares an authored bound against the ABSENT half\'s default', () => {
    ...
    expectDiagnostic(scene(node('PointLight2D', { range_z_max: -2000 })), {
      ruleName: 'pointlight2d-inverted-z-range',
      severity: 'warning',
    });
    expectDiagnostic(scene(node('PointLight2D', { range_layer_min: 1 })), {
      ruleName: 'pointlight2d-inverted-layer-range',
      severity: 'warning',
    });
  });
  ...
  it('ignores a malformed bound, which the validators already report', () => {
    const diagnostics = lint(scene(node('PointLight2D', { range_z_min: '"five"' })));
    expect(
