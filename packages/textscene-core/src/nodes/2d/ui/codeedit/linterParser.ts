/**
 * CodeEdit's own validators, those doc/classes/CodeEdit.xml lists without `overrides=`. `layout_direction`
 * and `text_direction` only override defaults: `_bind_methods` (code_edit.cpp:2776-3081) never re-declares
 * them. The hand-rolled validators live in `delimiterValidators`, `prefixValidators`,
 * `bracePairValidators` and `arrayForms`, so this file stays the map of what CodeEdit declares.
 */
...
/**
 * `line_length_guidelines`: `set_line_length_guidelines` (code_edit.cpp:2499-2502) stores the array
 * verbatim, with no clamp, sort or uniqueness check, so no bound. Setter and getter are `TypedArray<int>`
 * (code_edit.h:505-506, code_edit.cpp:2499-2506) behind PACKED_INT32_ARRAY (code_edit.cpp:2981), so only
 * the packed form narrows to int32 (variant_parser.cpp:1428-1430). See {@link PACKED_INT32_ARRAY_RE}.
 */
function lineLengthGuidelinesValidator(): PropertyValidator {
  ...
  const validator = accepts((key, value, line) => {
    ...
    if (body === '') return null;
    // One pass: unreadable by the tokenizer, or read and then narrowed away at
    // the width the matched spelling converts through.
...
// Grouped as `_bind_methods`'s ADD_GROUP structure (code_edit.cpp:2977-3013). TextEdit's keys
// arrive through the NODE_BASE_TYPES walk, and a re-declared one shadows it.
validatorRegistry.registerAll('CodeEdit', {
  ...
  // Code Completion (ADD_GROUP "Code Completion", "code_completion_", code_edit.cpp:3000-3002).
  // code_edit.cpp:3001, BOOL, PROPERTY_HINT_GROUP_ENABLE: an inspector hint, not a bound.
  // set_code_completion_enabled (code_edit.cpp:2204-2206) is a bare assignment.
