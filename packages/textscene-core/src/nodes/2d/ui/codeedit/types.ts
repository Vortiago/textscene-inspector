/** CodeEdit properties: TextEdit plus the gutters this slice draws. */
...
export interface CodeEditProperties extends TextEditProperties {
   ...
   * `set_indent_size` forwards to `TextEdit::set_tab_size` (`code_edit.cpp:908-920`),
   * which re-aligns every tab glyph to an `indent_size`-wide stop (`textEditTabStopsPx`).
   */
  indentSize?: number;
