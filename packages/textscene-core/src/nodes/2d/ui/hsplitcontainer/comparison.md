...
Strict parsing format-checks the inherited set (10 inherited from SplitContainer, 53 inherited from Control, 16 inherited from CanvasItem, 10 inherited from Node); `HSplitContainer` declares none of its own. Every validator failure is an **error**. `HSplitContainer` also REFUSES `vertical`, which its base declares but this class cannot carry.
