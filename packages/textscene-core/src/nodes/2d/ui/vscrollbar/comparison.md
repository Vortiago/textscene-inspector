...
Godot draws VScrollBar as a vertical track with increment/decrement buttons and a draggable grabber, going from top (min) to bottom (max); the previewer parses and validates this node but does not draw it yet, so it renders as an invisible transform-only fallback and its children still show.
...
Strict parsing format-checks the inherited set (1 inherited from ScrollBar, 9 inherited from Range, 28 inherited from Control, 15 inherited from CanvasItem, 10 inherited from Node); `VScrollBar` declares none of its own. Every validator failure is an **error**.
...
VScrollBar's lenient parser is `parseControl`, which has no field for `custom_step`,
`page`, `value` or any other Range/ScrollBar member. A malformed `value =
