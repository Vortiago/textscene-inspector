...
/**
 * One `connections` entry, as `set_connections` (`graph_edit.cpp:2533-2543`)
 * reads it off each Dictionary. `keep_alive` is left out: it never changes
 * what a static load draws.
 */
...
/**
 * GraphEdit's own members the renderer reads. `type_names`, `panning_scheme`,
 * `right_disconnects` and `zoom_step` reach no frozen frame, so the renderer
 * skips them. `linterParser.ts` validates them. `scrollOffset`, `zoom` and the `zoom*Disabled`
 * flags hold `loadOrder.ts`'s replay, since each setter reads an earlier key.
 */
