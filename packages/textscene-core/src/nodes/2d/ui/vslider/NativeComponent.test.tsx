/**
 * `<VSliderNative>` — the axis-swapped twin of `hslider/NativeComponent.test.tsx`
...
describe('<VSliderNative>', () => {
  it('draws exactly three meshes (track, fill, grabber) with no tick_count', async () => {
    const renderer = await ReactThreeTestRenderer.create(
      <VSliderNative {...painterEnv()} solveNode={solveNode({})} rect={RECT} renderOrder={0} />
    );
  ...
  it('puts the grabber flush TOP at value=max_value', async () => {
    const renderer = await ReactThreeTestRenderer.create(
      <VSliderNative {...painterEnv()} solveNode={solveNode({ value: 100 })} rect={RECT} renderOrder={0} />
    );
    const groups = renderer.scene.findAllByType('Group');
    const grabberGroup = groups[groups.length - 1]!;
  ...
  it('forwards renderOrder to every mesh it draws', async () => {
    const renderer = await ReactThreeTestRenderer.create(
      <VSliderNative {...painterEnv()} solveNode={solveNode({ tickCount: 3, ticksOnBorders: true })} rect={RECT} renderOrder={9} />
    );
