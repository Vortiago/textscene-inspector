...
<!-- lint:begin VSlider -->
Strict parsing format-checks the inherited set (5 inherited from Slider, 9 inherited from Range, 27 inherited from Control, 15 inherited from CanvasItem, 10 inherited from Node); `VSlider` declares none of its own. Every validator failure is an **error**.
