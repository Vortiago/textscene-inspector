/**
 * GraphElement strict validators, asserted through `validatorRegistry` so a
 * failure points at the validator, not at scene parsing. Each member is a plain
 * Vector2 or bool with no bound (scene/gui/graph_element.cpp), so each is
 * format-only. linter.test.ts covers the selectable/selected rule.
 */
...
describe('GraphElement strict validators', () => {
  ...
  it('rejects a malformed value on every property it validates', () => {
    ...
    // check is generic on purpose; per-property cases below are specific.
