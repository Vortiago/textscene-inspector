/**
 * TextureButton registration: parsed and validated, not yet rendered.
 *
...
describe('TextureButton registration', () => {
  it('registers the parseControl parse it reuses', () => {
    const registration = nodeRegistry.getRegistration('TextureButton');
    expect(registration).not.toBeNull();
    expect(registration!.parser).toBe(parseControl);
  });

  it('registers no render component, so it still reads as not implemented', () => {
    expect(nodeComponentRegistry.get('TextureButton')).toBeUndefined();
  });
  ...
  it('lands in the lenient parser tree with its type preserved and no fallback warning', () => {
    ...
    const scene = new TscnParser().parse(
      '[gd_scene format=3]\n\n[node name="Root" type="Control"]\n\n' +
        '[node name="MyTextureButton" type="TextureButton" parent="."]\n'
    );

    const node = scene.nodes[0]?.children[0];
    expect(node?.type).toBe('TextureButton');
