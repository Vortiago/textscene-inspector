...
Strict parsing format-checks the inherited set (3 inherited from FlowContainer, 53 inherited from Control, 16 inherited from CanvasItem, 10 inherited from Node); `HFlowContainer` declares none of its own. Every validator failure is an **error**. `HFlowContainer` also REFUSES `vertical`, which its base declares but this class cannot carry.
