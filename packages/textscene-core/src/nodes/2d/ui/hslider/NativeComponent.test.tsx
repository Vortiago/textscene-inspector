/**
 * `<HSliderNative>` — pins the four parts `Slider::_notification(NOTIFICATION_DRAW)`
...
describe('<HSliderNative>', () => {
  it('draws exactly three meshes (track, fill, grabber) with no tick_count', async () => {
    const renderer = await ReactThreeTestRenderer.create(
      <HSliderNative {...painterEnv()} solveNode={solveNode({})} rect={RECT} renderOrder={0} />
    );
  ...
  it('puts the grabber flush RIGHT at value=max_value — right edge === rect width', async () => {
    const renderer = await ReactThreeTestRenderer.create(
      <HSliderNative {...painterEnv()} solveNode={solveNode({ value: 100 })} rect={RECT} renderOrder={0} />
    );
    const groups = renderer.scene.findAllByType('Group');
    const grabberGroup = groups[groups.length - 1]!;
  ...
  it('wears the DISABLED grabber texture when editable is false', async () => {
    ...
    const disabled = await ReactThreeTestRenderer.create(
      <HSliderNative {...painterEnv()} solveNode={solveNode({ editable: false })} rect={RECT} renderOrder={0} />
    );
  ...
  it('forwards renderOrder to every mesh it draws', async () => {
    const renderer = await ReactThreeTestRenderer.create(
      <HSliderNative {...painterEnv()} solveNode={solveNode({ tickCount: 3, ticksOnBorders: true })} rect={RECT} renderOrder={7} />
    );
