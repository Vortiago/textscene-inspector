...
Strict parsing format-checks the inherited set (10 inherited from SplitContainer, 28 inherited from Control, 15 inherited from CanvasItem, 10 inherited from Node); `VSplitContainer` declares none of its own. Every validator failure is an **error**. `VSplitContainer` also REFUSES `vertical`, which its base declares but this class cannot carry.
