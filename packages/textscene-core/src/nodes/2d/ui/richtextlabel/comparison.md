...
- **Approximated** Fill alignment is not justified, so a filled paragraph keeps a ragged
  right edge instead of stretching to the box. A justified line needs per-run glyph
  slicing that re-derives its run boundaries, and that is not wired to it.
- **Not drawn** Every `_draw_line` RTL arm (rich_text_label.cpp:987-1014): the
  ...
  `TEXT_DIRECTION_AUTO` (rich_text_label.h:615), and `[p dir=]` is not modelled. In the
  engine, an RTL RichTextLabel draws its lines where an LTR one does.
