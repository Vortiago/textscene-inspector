 ...
 * RichTextLabel: the bbcode subset `[b]`/`[i]`/`[color]` (ADR-0003; anything
 ...
 * (`native/text/textLayout.ts` + `TextRun.tsx`), the same engine `<LabelNative>`
 * draws through.
 *
...
export function RichTextLabelNative({ solveNode, rect, renderOrder, theme }: NativeControlComponentProps) {
  ...
  const runs = useMemo(() => styledTextRuns(props, textTheme.color), [props, textTheme.color]);
  ...
  const layout = useMemo(
    ...
    [plainText, textTheme.fontSizePx, rect.w, autowrapMode]
  );
