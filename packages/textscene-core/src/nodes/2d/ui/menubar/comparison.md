...
`flat`) and its text, left to right with `h_separation` between them. The popups never
open, since a PopupMenu is a Window and this previewer draws no Windows.
...
`linterParser.ts` format-checks all of MenuBar's own members. `parser.ts` reads
`flat`: a malformed value becomes `false`, with no warning. The others
...
Each title's text comes from its PopupMenu child's `title` property when set, else from
its node name, the same fallback as `MenuBar::_refresh_menu_names`. A title is always
drawn in Godot's plain "normal" state: per-menu `disabled`/`hidden` are runtime-only APIs
with no serialised property, so a `.tscn` cannot author them.
