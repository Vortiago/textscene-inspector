...
describe('TextEdit strict validators', () => {
  ...
  describe('caret_blink_interval (setter refuses <= 0, hint states 0.1 to 10)', () => {
    // text_edit.cpp:5198: ERR_FAIL_COND(p_interval <= 0). The
    // PROPERTY_HINT_RANGE "0.1,10,0.01,suffix:s" (text_edit.cpp:7583) is an
    // editor slider the setter never applies, so both of its ends warn.
