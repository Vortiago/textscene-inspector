...
<!-- lint:begin TextEdit -->
Strict parsing format-checks these `TextEdit` properties, plus 28 inherited from Control, 15 inherited from CanvasItem, 10 inherited from Node. Every validator failure is an **error**.
...
| `caret_type` | enum 0-1 (CARET_TYPE_LINE/CARET_TYPE_BLOCK) |
...
| `wrap_mode` | enum 0-1 (LINE_WRAPPING_NONE/LINE_WRAPPING_BOUNDARY) |
