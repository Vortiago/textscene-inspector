/**
 * <ControlNative> — the native (WebGL canvas) painter for a plain `Control`.
 ...
 *    scale-about-pivot transform (`controlLayout.ts:211-216`'s rule, applied
 *    by the walker around EVERY registered painter) reaches a real `Control`
 *    node once one is registered — proving the walker's plumbing end to end
 *    for the simplest possible painter.
 */
...
describe('<ControlNative> (isolated painter contract)', () => {
...
describe('<ControlNative> registered through <ControlCanvasWalker> (end-to-end walker plumbing)', () => {
  afterEach(() => {
  ...
  it('replaces the debug fallback outline with no visible chrome once registered', async () => {
    controlComponentRegistry.register({ typeName: 'Control', Component: DomStub, Native: ControlNative });
