...
walk (`title`/`tooltip`/`icon`/`disabled`), aligned to `tab_count` like OptionButton's
...
- **Approximated** `max_tab_width` caps a tab's on-screen width and reserves
  the same pixel budget Godot does, but the glyphs inside that budget draw unclipped
  rather than as Godot's `OVERRUN_TRIM_ELLIPSIS` truncation with a trailing "…".
