/**
 * `<OptionButtonNative>` — the native (WebGL canvas) painter for
 ...
 * chevron arrow icon at its right edge. This closes the missing-chevron row
 * `comparison.md` records (the DOM twin has no access to the theme's icon
 * texture at all).
 *
 * Tint follows `ButtonNative`'s rule: `modulate` is already folded into the
 ...
 * `tint.opacity` directly, same as `CheckBoxNative`'s icon.
 *
...
export function OptionButtonNative({ solveNode, rect, renderOrder, theme }: NativeControlComponentProps) {
