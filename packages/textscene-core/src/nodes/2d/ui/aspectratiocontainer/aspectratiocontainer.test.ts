/**
 * AspectRatioContainer registration — parsed and validated, not yet rendered.
 *
...
describe('AspectRatioContainer registration', () => {
  ...
  it('registers no render component, so it still reads as not implemented', () => {
    expect(nodeComponentRegistry.get('AspectRatioContainer')).toBeUndefined();
  });
  ...
  it('lands in the lenient parser tree with its type preserved and no fallback warning', () => {
    ...
    const scene = new TscnParser().parse(
      '[gd_scene format=3]\n\n[node name="Root" type="Control"]\n\n' +
        '[node name="MyAspectRatioContainer" type="AspectRatioContainer" parent="."]\n'
    );

    const node = scene.nodes[0]?.children[0];
    expect(node?.type).toBe('AspectRatioContainer');
