 ...
 * `SubViewportContainer`, the ONE viewport surface that still had no native
 * painter (Godot-parity table in `../../viewport/subviewport/comparison.md`).
 *
 * Unlike the DOM surface (`Component.tsx`), which snapshots a target into CPU
 * pixels via `readPixels`, this painter samples the published
 * `ViewportTextureEntry.texture` directly — every sub-viewport kind (3D,
 * 2D-world, and now the native Control-raster pass) publishes a WebGL
 * texture, so there is exactly one consumption path.
 */
...
describe('<SubViewportContainerNative>', () => {
