/**
 * `<CanvasLayerNative>` render contract: the native (WebGL canvas) counterpart
 * of `./Component.tsx`'s DOM `<CanvasLayer>`. Draws no chrome of its own — it
 * only republishes fresh `CanvasLayerIndexProvider`/`CanvasModulateContext`
 * scopes around its children, mirroring the `CanvasLayer` branch of
 * `NodeDispatcher.tsx`'s `PlainNode` (see that module — this is the same
 * convention, not a second one). Assertions read scene-graph structure only,
 * never pixels, matching `nodes/2d/marker2d/Component.test.tsx`'s style for a
