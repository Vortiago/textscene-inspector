...
<!-- lint:begin CheckBox -->
Strict parsing format-checks the inherited set (13 inherited from Button, 10 inherited from BaseButton, 28 inherited from Control, 16 inherited from CanvasItem, 10 inherited from Node); `CheckBox` declares none of its own. Every validator failure is an **error**.
