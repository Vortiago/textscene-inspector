...
Strict parsing format-checks these `ColorPicker` properties, plus 1 inherited from BoxContainer, 28 inherited from Control, 15 inherited from CanvasItem, 10 inherited from Node. Every validator failure is an **error**. `ColorPicker` also REFUSES `vertical`, which its base declares but this class cannot carry.
