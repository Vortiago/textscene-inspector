/**
 * MultiMeshInstance2D registration: parsed and validated, not yet rendered. The slice
...
describe('MultiMeshInstance2D registration', () => {
  ...
  it('registers a base component as a declared gap, so it still reads as not implemented', () => {
    expect(nodeComponentRegistry.renderIntentOf('MultiMeshInstance2D')).toBe('pending');
    expect(rendersOwnVisual('MultiMeshInstance2D')).toBe('not-implemented');
  });
});
