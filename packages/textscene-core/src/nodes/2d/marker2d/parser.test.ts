...
function heading(name = 'MyMarker'): ParsedHeading {
  return { type: 'node', attributes: { type: 'Marker2D', name } };
}

describe('parseMarker2D', () => {
  it('defaults gizmo_extents to 10 and inherits the Node2D transform', () => {
    const props = parseMarker2D(heading(), { position: 'Vector2(32, 64)' });
  ...
  it('tolerates a non-numeric gizmo_extents (edge) → default', () => {
    const props = parseMarker2D(heading(), { gizmo_extents: 'not-a-number' });
    expect(props.gizmo_extents).toBe(10);
  });
});
