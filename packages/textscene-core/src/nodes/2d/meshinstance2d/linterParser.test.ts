/**
 * MeshInstance2D strict validators — format and range checks.
 *
 ...
 * MeshInstance2D binds, or set DECLARES_NOTHING when it binds no ADD_PROPERTY at all.
 * Leaving both unset is red on purpose. Do NOT delete an assertion to go green.
 *
 * mesh_instance_2d.cpp:64-65 — the only two ADD_PROPERTY calls in
 * MeshInstance2D::_bind_methods, and the only members doc/classes/MeshInstance2D.xml
 * lists. No PropertyListHelper/register_property, no ADD_ARRAY_COUNT, no
 * `_set`/`_get`/`get_property_list` override anywhere in mesh_instance_2d.cpp or .h.
 */
 ...
 * an EMPTY set and passes while asserting nothing — "Godot gives MeshInstance2D no
...
describe('MeshInstance2D strict validators', () => {
  ...
  it('rejects a malformed value on every property it validates', () => {
    ...
    // MeshInstance2D declares nothing, which is what INHERITED below covers.
  ...
  it('resolves each inherited key to the ancestor that declares it', () => {
    ...
    for (const [owner, key] of INHERITED) {
      ...
      // MeshInstance2D would answer here while drifting from the ancestor's rule.
