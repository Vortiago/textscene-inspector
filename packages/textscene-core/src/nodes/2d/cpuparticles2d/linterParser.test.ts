/**
 * CPUParticles2D strict validators: format and range checks, asserted through
 * `validatorRegistry`. Assertions check `error?.code`, the code the `v` DSL derives
 * (`INVALID_<NAME>_FORMAT` / `INVALID_<NAME>_VALUE`), not the message text.
 */
...
describe('CPUParticles2D strict validators', () => {
  ...
  describe('draw_order', () => {
    ...
    // 173-175) is `draw_order = p_order;` with no ERR_FAIL_INDEX and no CLAMP,
    ...
    it('warns (not errors) on the value scenes/demos/2d/platformer/enemy/enemy.tscn:296 ships (`draw_order = 215832976`)', () => {
      // Godot reads anything but 1 as Index, so an error here would fail a scene
    ...
    it('is NOT the same enum as GPUParticles2D.draw_order, which accepts a third value (2) this one warns on', () => {
      // gpu_particles_2d.h:40-43 adds DRAW_ORDER_REVERSE_LIFETIME = 2, but
  ...
  describe('hue_variation_min / hue_variation_max', () => {
    // cpu_particles_2d.cpp:1646-1647 hint "-1,1,0.01", closed both ends.
    // set_param_min/set_param_max (cpu_particles_2d.cpp: 352, 367) ERR_FAIL_INDEX
    // the Parameter enum, never the value, so out-of-hint is a warning. Each
    // endpoint is probed from both sides to pin where the bound sits.
  ...
  describe('fixed_fps', () => {
    // cpu_particles_2d.cpp:1504 hints "0,1000,1,suffix:FPS": both ends closed,
    // and `suffix:FPS` is a unit. set_fixed_fps (cpu_particles_2d.cpp:283-285) is
    // `fixed_fps = p_count;`, so out of hint warns at both ends (ADR-0032). Each
    // endpoint is probed from both sides.
  ...
  describe('emission_sphere_radius', () => {
    // cpu_particles_2d.cpp:1587 hints "0.01,128,0.01,suffix:px": both ends
    // closed, and `suffix:px` is a unit. set_emission_sphere_radius
    // (cpu_particles_2d.cpp:491-499) assigns unconditionally, so out of hint warns
    // at both ends (ADR-0032), as in the CPUParticles3D twin.
  ...
  describe('seed', () => {
    // cpu_particles_2d.cpp:1502 hints "0," + itos(UINT32_MAX) + ",1", closed both
    // ends. set_seed (cpu_particles_2d.cpp:613) is `seed = p_seed;` on a uint32_t
    // param, which coerces rather than refuses, so both ends warn.
  ...
  describe('anim_offset_min / anim_offset_max', () => {
    // cpu_particles_2d.cpp:1653-1654 hint "0,1,0.0001", closed both ends. The same
    // index-only set_param_min/set_param_max guard (:353, :368) leaves the value
    // unchecked, so both ends warn.
  ...
  describe('emission_colors', () => {
    ...
    // so this is a genuine PackedColorArray: format-only, no range.
