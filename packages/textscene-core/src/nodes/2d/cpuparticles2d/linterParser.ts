/** CPUParticles2D strict validators: property format and bound checks. */
...
// cpu_particles_2d.cpp:1509 hints 2 labels ("Index,Lifetime"), and
// BIND_ENUM_CONSTANT binds both (:1511-1512). GPUParticles2D's DrawOrder
// (gpu_particles_2d.h:40-43) is another enum, with a third value, REVERSE_LIFETIME.
...
validatorRegistry.registerAll('CPUParticles2D', {
  ...
  // set_amount ERR_FAIL_COND_MSGs below 1 (cpu_particles_2d.cpp:68), so the floor
  // is enforced. The setter never checks the ceiling, so it warns, as in the
  // CPUParticles3D twin.
  amount: v.int('amount', {
    min: 1,
    max: 1000000,
  ...
  // ends. set_seed (cpu_particles_2d.cpp:613) is `seed = p_seed;`: the uint32_t
  // param coerces an out-of-range value rather than rejecting it.
  ...
  // set_draw_order (cpu_particles_2d.cpp:173-175) is `draw_order = p_order;` with
  // no ERR_FAIL_INDEX and no CLAMP, so out of range is a warning: the engine
  // reads `draw_order = 215832976` as Index.
  ...
  // cpu_particles_2d.cpp:1593 carries no hint, and set_emission_ring_radius
  // (cpu_particles_2d.cpp:531-533) assigns unconditionally: no bound.
  ...
  // not a `TypedArray<Color>`, so Godot writes a PackedColorArray, not
  // `Array[Color]([…])`.
  ...
  // 367) only ERR_FAIL_INDEX the Parameter enum, never the value.
