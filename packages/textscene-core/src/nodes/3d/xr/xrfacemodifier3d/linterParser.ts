/**
 * XRFaceModifier3D strict validators for linting.
 *
 * Declare only XRFaceModifier3D's OWN members — the ones doc/classes/XRFaceModifier3D.xml
...
validatorRegistry.registerAll('XRFaceModifier3D', {
  // xr_face_modifier_3d.cpp:498, declared Variant::STRING with
  // PROPERTY_HINT_ENUM_SUGGESTION "/user/face_tracker" (a picker default, not
  // a format), but set_face_tracker takes `const StringName &` and
  // get_face_tracker returns `StringName` (cpp:505-515), so Godot saves the
  // &"…" spelling — the getter's type, not the declared one, decides the
  // serialised form. set_face_tracker assigns straight through, so no bound.
