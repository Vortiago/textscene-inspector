/**
 * XRAnchor3D strict validators, asserted through `validatorRegistry`, not by
...
/**
 * The keys XRAnchor3D binds, read from the source. Set this or DECLARES_NOTHING,
 * not both. Leaving both unset is red on purpose: do not delete an assertion to go green.
 */
const KEYS: string[] = [];
/**
 * True: XRAnchor3D binds nothing beyond XRNode3D. `xr_nodes.h:163-175` is the whole class body, and
 * `xr_nodes.cpp:660-663`'s `_bind_methods` binds only `get_size`/`get_plane`, with no property-list
 * override in either spelling. `doc/classes/XRAnchor3D.xml` carries no `<members>` block.
 */
const DECLARES_NOTHING = true;

/**
 * Keys XRAnchor3D does not declare, each paired with the ancestor that does. The
...
describe('XRAnchor3D strict validators', () => {
  ...
  it('accepts every value its own fixture carries', () => {
    ...
    // parent chain, so it covers what XRNode3D up declares until KEYS gains an entry.
