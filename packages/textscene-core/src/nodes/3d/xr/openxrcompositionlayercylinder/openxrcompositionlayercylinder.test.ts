/**
 * OpenXRCompositionLayerCylinder registration — parsed, and deliberately NOT given a
 * render component.
 *
...
describe('OpenXRCompositionLayerCylinder registration', () => {
  ...
  it('registers NO component, so GenericNodeFallback keeps handling it', () => {
    ...
    // the two agree.
    expect(nodeComponentRegistry.get('OpenXRCompositionLayerCylinder')).toBeUndefined();
    expect(nodeComponentRegistry.isTransformOnly('OpenXRCompositionLayerCylinder')).toBe(false);
  });
  ...
  it('lands in the lenient parser tree with its type preserved and no fallback warning', () => {
    ...
    const scene = new TscnParser().parse(
      '[gd_scene format=3]\n\n[node name="Root" type="Node3D"]\n\n' +
        '[node name="MyOpenXRCompositionLayerCylinder" type="OpenXRCompositionLayerCylinder" parent="."]\n'
    );

    const node = scene.nodes[0]?.children[0];
    expect(node?.type).toBe('OpenXRCompositionLayerCylinder');
