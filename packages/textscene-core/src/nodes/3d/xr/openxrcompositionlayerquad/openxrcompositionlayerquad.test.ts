/**
 * OpenXRCompositionLayerQuad registration: parsed, its own visual still a gap.
 *
...
describe('OpenXRCompositionLayerQuad registration', () => {
  ...
  it('registers a base component as a declared gap, so it still reads as not implemented', () => {
    ...
    expect(nodeComponentRegistry.renderIntentOf('OpenXRCompositionLayerQuad')).toBe('pending');
    expect(rendersOwnVisual('OpenXRCompositionLayerQuad')).toBe('not-implemented');
  });
