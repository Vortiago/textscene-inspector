/**
 * OpenXRCompositionLayerQuad registration — parsed, and deliberately NOT given a
 * render component.
 *
...
describe('OpenXRCompositionLayerQuad registration', () => {
  it('registers the parseNode3D parse it reuses', () => {
    const registration = nodeRegistry.getRegistration('OpenXRCompositionLayerQuad');
  ...
  it('registers NO component, so GenericNodeFallback keeps handling it', () => {
    ...
    // the two agree.
    expect(nodeComponentRegistry.get('OpenXRCompositionLayerQuad')).toBeUndefined();
    expect(nodeComponentRegistry.isTransformOnly('OpenXRCompositionLayerQuad')).toBe(false);
  });
  ...
  it('lands in the lenient parser tree with its type preserved and no fallback warning', () => {
    ...
    const scene = new TscnParser().parse(
      '[gd_scene format=3]\n\n[node name="Root" type="Node3D"]\n\n' +
        '[node name="MyOpenXRCompositionLayerQuad" type="OpenXRCompositionLayerQuad" parent="."]\n'
    );

    const node = scene.nodes[0]?.children[0];
    expect(node?.type).toBe('OpenXRCompositionLayerQuad');
