/**
 * The three visibility-range warnings of GeometryInstance3D::get_configuration_warnings
 * (scene/3d/visual_instance_3d.cpp). Its two `forward_plus` warnings are not
 * ported: they read the project's renderer setting, which a single `.tscn` lacks.
 */
...
function checkGeometryInstance3D(context: RuleContext): Diagnostic[] {
  ...
  // `CSGShape3D::get_configuration_warnings` chains to `Node`'s (csg_shape.cpp:978),
  // so no warning here reaches a CSG shape. Every other override chains through
  // this class (gpu_particles_3d.cpp:337, cpu_particles_3d.cpp:215,
  ...
  if (!isZeroish(props.visibility_range_end)) {
    ...
    // No finiteness guard: `end <= begin` is false when either side is `nan`,
...
const geometryInstance3DValidationRule: LintRule = {
  meta: {
    ...
    category: 'validation',
    // `descendsFrom`, not `applicableNodeTypes`: RuleRegistry matches that by exact
