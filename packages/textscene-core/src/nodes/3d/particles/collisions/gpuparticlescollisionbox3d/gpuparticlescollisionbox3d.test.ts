/**
 * GPUParticlesCollisionBox3D registration — parsed and validated, not yet rendered.
 *
...
describe('GPUParticlesCollisionBox3D registration', () => {
  it('registers the Node3D base parser', () => {
    const registration = nodeRegistry.getRegistration('GPUParticlesCollisionBox3D');
  ...
  it('registers a base component as a declared gap, so it still reads as not implemented', () => {
    expect(nodeComponentRegistry.renderIntentOf('GPUParticlesCollisionBox3D')).toBe('pending');
    expect(rendersOwnVisual('GPUParticlesCollisionBox3D')).toBe('not-implemented');
  });
  ...
  it('lands in the lenient parser tree with its type preserved and no fallback warning', () => {
    ...
    const scene = new TscnParser().parse(
      '[gd_scene format=3]\n\n[node name="Root" type="Node3D"]\n\n' +
        '[node name="MyGPUParticlesCollisionBox3D" type="GPUParticlesCollisionBox3D" parent="."]\n'
    );

    const node = scene.nodes[0]?.children[0];
    expect(node?.type).toBe('GPUParticlesCollisionBox3D');
