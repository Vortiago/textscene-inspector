/**
 * CPUParticles3D registration — parsed and validated, not yet rendered.
 *
...
describe('CPUParticles3D registration', () => {
  ...
  it('registers a base component as a declared gap, so it still reads as not implemented', () => {
    expect(nodeComponentRegistry.renderIntentOf('CPUParticles3D')).toBe('pending');
    expect(rendersOwnVisual('CPUParticles3D')).toBe('not-implemented');
  });
  ...
  it('lands in the lenient parser tree with its type preserved and no fallback warning', () => {
    ...
    const scene = new TscnParser().parse(
      '[gd_scene format=3]\n\n[node name="Root" type="Node3D"]\n\n' +
        '[node name="MyCPUParticles3D" type="CPUParticles3D" parent="."]\n'
    );

    const node = scene.nodes[0]?.children[0];
    expect(node?.type).toBe('CPUParticles3D');
