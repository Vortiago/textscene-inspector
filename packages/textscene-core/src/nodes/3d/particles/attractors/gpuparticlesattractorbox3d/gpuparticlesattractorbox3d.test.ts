...
describe('GPUParticlesAttractorBox3D registration', () => {
  ...
  it('registers no render component, so it still reads as not implemented', () => {
    expect(nodeComponentRegistry.get('GPUParticlesAttractorBox3D')).toBeUndefined();
  });
  ...
  it('lands in the lenient parser tree with its type preserved and no fallback warning', () => {
    ...
    const scene = new TscnParser().parse(
      '[gd_scene format=3]\n\n[node name="Root" type="Node3D"]\n\n' +
        '[node name="MyGPUParticlesAttractorBox3D" type="GPUParticlesAttractorBox3D" parent="."]\n'
    );

    const node = scene.nodes[0]?.children[0];
    expect(node?.type).toBe('GPUParticlesAttractorBox3D');
