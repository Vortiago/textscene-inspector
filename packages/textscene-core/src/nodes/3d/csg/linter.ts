 ...
 * `polygon` has fewer than 3 points, or a zero/negative `size` component on a
...
function checkCSGShape3D(context: RuleContext): Diagnostic[] {
  ...
  // non-positive component is degenerate.
    ...
    try {
      ...
      if (size.x <= 0 || size.y <= 0 || size.z <= 0) {
        return [
          {
            severity: 'warning',
            message: `CSGBox3D '${node.name}' has a zero or negative size component (${properties.size}), so it has no volume.`,
            nodeName: node.name,
            nodeType: node.type,
...
const csgShape3DDegenerateGeometryRule: LintRule = {
  meta: {
    ...
    description:
      'Flags a CSG leaf whose own declared geometry is degenerate: a CSGMesh3D with no mesh, a CSGPolygon3D under 3 points, or a non-positive CSGBox3D size',
    category: 'validation',
