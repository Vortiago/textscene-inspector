...
validatorRegistry.registerAll('CSGSphere3D', {
  ...
  // csg_shape.cpp:1478, ERR_FAIL_COND(p_radius <= 0): the setter refuses the
  // floor and assigns anything above it straight through, so the 100 ceiling
  // is the hint's alone and warns.
  radius: v.float('radius', {
    min: Number.MIN_VALUE,
    max: 100.0,
    enforced: { min: 'csg_shape.cpp:1478' },
    hinted: { max: 'csg_shape.cpp:1470' },
    message: "Property 'radius' must be greater than 0 and no more than 100",
  }),
