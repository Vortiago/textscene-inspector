/**
 * VoxelGI registration — parsed and validated, not yet rendered.
 *
...
describe('VoxelGI registration', () => {
  it('registers the parseNode3D parse it reuses', () => {
    const registration = nodeRegistry.getRegistration('VoxelGI');
  ...
  it('registers a base component as a declared gap, so it still reads as not implemented', () => {
    expect(nodeComponentRegistry.renderIntentOf('VoxelGI')).toBe('pending');
    expect(rendersOwnVisual('VoxelGI')).toBe('not-implemented');
  });
  ...
  it('lands in the lenient parser tree with its type preserved and no fallback warning', () => {
    ...
    const scene = new TscnParser().parse(
      '[gd_scene format=3]\n\n[node name="Root" type="Node3D"]\n\n' +
        '[node name="MyVoxelGI" type="VoxelGI" parent="."]\n'
    );

    const node = scene.nodes[0]?.children[0];
    expect(node?.type).toBe('VoxelGI');
