...
// an inherited key only if the ancestor is pulled in too.
...
// camera_3d.h:50-52: `enum KeepAspect { KEEP_WIDTH, KEEP_HEIGHT };` has two members, and
// camera_3d.cpp:672's hint offers only "Keep Width,Keep Height". No "KEEP_ASPECT_DISABLED"
// exists in Godot 4.6.3, so a value of 2 is one the engine cannot represent.
...
validatorRegistry.registerAll('Camera3D', {
  ...
  // Two tiers on the floor. camera_3d.cpp:731 `ERR_FAIL_COND(p_size <= CMP_EPSILON)` refuses the
  // endpoint too, hence `exclusive`. The hint (:683, "0.001,100,0.001,or_greater,suffix:m")
  // states 0.001, so the band between them loads and only warns. `or_greater` opens the ceiling.
  size: v.float('size', {
