...
<!-- lint:begin Camera3D -->
Strict parsing format-checks these `Camera3D` properties, plus 17 inherited from Node3D, 10 inherited from Node. Every validator failure is an **error**.
...
| `h_offset` | float |
| `keep_aspect` | enum 0-1 (KEEP_WIDTH/KEEP_HEIGHT) |
| `near` | float |
