 ...
 * each forwarded untouched to a modern setter. `godot/deprecated.ts` resolves
 * all three, so the property bag and the strict parser's validator lookup both
 * see the modern key and nothing registers here under the old one.
 */
