/**
 * LightmapProbe registration: parsed and validated, not yet rendered. The base
...
describe('LightmapProbe registration', () => {
  ...
  it('registers a base component as a declared gap, so it still reads as not implemented', () => {
    expect(nodeComponentRegistry.renderIntentOf('LightmapProbe')).toBe('pending');
    expect(rendersOwnVisual('LightmapProbe')).toBe('not-implemented');
  });
});
