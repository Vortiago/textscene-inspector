...
<!-- lint:begin FABRIK3D -->
Strict parsing format-checks the inherited set (6 inherited from IterateIK3D, 1 inherited from IKModifier3D, 2 inherited from SkeletonModifier3D, 16 inherited from Node3D, 10 inherited from Node); `FABRIK3D` declares none of its own. Every validator failure is an **error**.
