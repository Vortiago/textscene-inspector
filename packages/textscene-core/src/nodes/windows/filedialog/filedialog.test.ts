/**
 * FileDialog registration — parsed and validated, not yet rendered.
 *
...
describe('FileDialog registration', () => {
  ...
  it('registers no render component, so it still reads as not implemented', () => {
    expect(nodeComponentRegistry.get('FileDialog')).toBeUndefined();
  });
  ...
  it('lands in the lenient parser tree with its type preserved and no fallback warning', () => {
    ...
    const scene = new TscnParser().parse(
      '[gd_scene format=3]\n\n[node name="Root" type="Node"]\n\n' +
        '[node name="MyFileDialog" type="FileDialog" parent="."]\n'
    );

    const node = scene.nodes[0]?.children[0];
    expect(node?.type).toBe('FileDialog');
