...
function checkPopupMenu(context: RuleContext): Diagnostic[] {
  ...
  // Resolved the way `_get_property` does, not by a hand-rolled key scan, so a key
  // with no leaf, such as `item_3/`, names no item. The dispatcher in linterParser.ts
  // reports a negative index, so this checks only the high end, and one mistake gets
  // one diagnostic.
