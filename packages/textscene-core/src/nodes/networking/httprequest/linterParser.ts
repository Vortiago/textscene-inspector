/**
 * HTTPRequest strict validators for linting.
 *
 * Declare only HTTPRequest's OWN members — the ones doc/classes/HTTPRequest.xml
 * lists without an `overrides=` attribute. Everything from Node up is
 * registered on the ancestor and delivered by the NODE_BASE_TYPES base-walk, so
 * re-declaring an inherited key shadows it and duplicates the rule.
 *
 * HTTPRequest declares no `get_configuration_warnings()` override at all
 * (confirmed by reading http_request.cpp/.h in full) — nothing to implement or
 * decline there.
 */
...
validatorRegistry.registerAll('HTTPRequest', {
  ...
  // http_request.cpp:673, PROPERTY_HINT_RANGE "-1,2000000000,suffix:B" (both
  // ends closed). set_body_size_limit (:557-561) only ERR_FAIL_CONDs on
  // connection state, then assigns straight through — the hint is the only
  // authority on the value itself. -1 is the documented "no limit" sentinel,
  // not a floor to reject.
  body_size_limit: v.int('body_size_limit', { min: -1, max: 2000000000, hinted: 'http_request.cpp:673' }),
  // http_request.cpp:670, PROPERTY_HINT_RANGE "256,16777216,suffix:B" (both
  // ends closed). set_download_chunk_size (:577-581) forwards to the
  // underlying HTTPClientTCP::set_read_chunk_size (http_client_tcp.cpp:761-764),
  // whose ERR_FAIL_COND(p_size < 256 || p_size > (1 << 24)) enforces the exact
  // same bounds the hint states, so both ends are errors.
