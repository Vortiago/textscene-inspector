/**
 * Node3D transform format checks. `transform` is declared on the Node3D tier, so the base walk
 * delivers it to every spatial node. Driven through `./linterParser` rather than `Linter`, so a
...
describe('Node3D transform validators', () => {
  ...
  describe('rotation_edit_mode', () => {
    ...
    // `ERR_FAIL_INDEX(int32_t(p_order), 6)`: two adjacent enums in the same file
    // with different tiers. Out of range is a warning, not an error.
    it('warns rather than errors outside the enum, unlike rotation_order', () => {
      ...
      // The code, not only the severity, proves the enum-range branch ran.
