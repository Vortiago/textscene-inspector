...
describe('parseNode2D', () => {
  ...
  it('converts rotation_degrees to radians', () => {
    expect(parseNode2D(heading(), { rotation_degrees: '90' }).rotation).toBeCloseTo(Math.PI / 2);
  });
  ...
  it('transform= wins over discrete props and is decomposed', () => {
    ...
    const p = parseNode2D(heading(), { transform: 'Transform2D(2, 0, 0, 3, 10, 20)', position: 'Vector2(99, 99)' });
