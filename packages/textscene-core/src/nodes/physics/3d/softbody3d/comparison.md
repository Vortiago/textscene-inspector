...
Strict parsing format-checks these `SoftBody3D` properties, plus 4 inherited from MeshInstance3D, 17 inherited from GeometryInstance3D, 1 inherited from VisualInstance3D, 16 inherited from Node3D. Every validator failure is an **error**.
