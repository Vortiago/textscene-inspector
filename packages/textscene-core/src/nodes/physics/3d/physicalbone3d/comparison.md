...
Strict parsing format-checks these `PhysicalBone3D` properties, plus 6 inherited from CollisionObject3D, 16 inherited from Node3D, 10 inherited from Node. Every validator failure is an **error**.
...
| `joint_type` | enum 0-5 (NONE/PIN/CONE/HINGE/SLIDER/6DOF) |
| `linear_damp` | float >= 0 |
