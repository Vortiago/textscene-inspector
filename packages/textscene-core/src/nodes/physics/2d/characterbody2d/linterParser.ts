...
validatorRegistry.registerAll('CharacterBody2D', {
   ...
   * Must be Vector2(x, y) - defines which way is "up"
   */
  'up_direction': (key, value, line) => {
    if (!VECTOR2_REGEX.test(value)) {
      return {
        severity: 'error',
        message: `Property 'up_direction' must be Vector2 with 2 numbers like Vector2(0, -1), got: "${value}"`,
        line,
        column: key.length + 3,
  ...
  'velocity': (key, value, line) => {
    if (!VECTOR2_REGEX.test(value)) {
      return {
        severity: 'error',
        message: `Property 'velocity' must be Vector2 with 2 numbers like Vector2(0, 0), got: "${value}"`,
        line,
        column: key.length + 3,
   ...
   * Godot stores angles in radians
   */
