...
Strict parsing format-checks these `PhysicalBone2D` properties, plus 14 inherited from RigidBody2D, 5 inherited from CollisionObject2D, 12 inherited from Node2D, 15 inherited from CanvasItem, 10 inherited from Node. Every validator failure is an **error**.

| Property | Accepts |
| --- | --- |
| `auto_configure_joint` | true or false |
| `bone2d_index` | integer -1-1000 |
