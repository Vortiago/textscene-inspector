/**
 * GrooveJoint2D registration — it is parsed, and it draws nothing on purpose
...
describe('GrooveJoint2D registration', () => {
  it('registers the Node2D base parser', () => {
    const registration = nodeRegistry.getRegistration('GrooveJoint2D');
    expect(registration).not.toBeNull();
    expect(registration!.parser).toBe(parseNode2D);
  });

  it('reuses the Node2D component so children keep their transform space', () => {
    expect(nodeComponentRegistry.get('GrooveJoint2D')).toBe(Node2D);
  });

  it('declares drawing nothing, so the sheet may claim linter-only', () => {
    expect(nodeComponentRegistry.isTransformOnly('GrooveJoint2D')).toBe(true);
  });
  ...
  it('lands in the lenient parser tree with its type preserved and no fallback warning', () => {
    ...
    const scene = new TscnParser().parse(
      '[gd_scene format=3]\n\n[node name="Root" type="Node2D"]\n\n' +
        '[node name="MyGrooveJoint2D" type="GrooveJoint2D" parent="."]\n'
    );

    const node = scene.nodes[0]?.children[0];
    expect(node?.type).toBe('GrooveJoint2D');
