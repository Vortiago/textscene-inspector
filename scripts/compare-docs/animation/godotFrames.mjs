...
export async function captureGodotFrames(fixture, framesDir, mode) {
  ...
  try {
    ...
    const ini = existsSync(join(root, 'project.godot'))
      ...
      : null;
    // A 3D animation renders Godot at the previewer canvas's frame (CANVAS_CAPTURE), as the still
    // 3D comparison does, so the two GIFs share a size and aspect and the gallery slider overlays
    // them 1:1 instead of stretching one into the other's box.
    const size =
      mode === '2d'
