...
// manifest. Games are written to a separate file (below) but share the name
// space at runtime, so check collisions across BOTH sets here.
const seenNames = new Map();
for (const fixture of [...fixtures, ...gameFixtures]) {
...
// The open-source games corpus is fetched on demand (`pnpm vendor:games`), not
// committed, so its manifest is written to a SEPARATE, gitignored file that
// fixturesAll.ts merges via import.meta.glob when present. On a fresh clone
// (no scenes/games/) gameFixtures is empty; remove any stale file so the
// committed state stays games-free.
const gamesOutputPath = join(rootDir, 'apps/textscene-web/src/fixtures.games.ts');
if (gameFixtures.length > 0) {
  const gameCategories = [...new Set(gameFixtures.map(f => f.category))];
  const sortedGameFixtures = [];
  gameCategories.forEach(cat => {
    sortedGameFixtures.push(...gameFixtures.filter(f => f.category === cat));
  });
  const gamesOutput = `/**
 * On-demand games-corpus manifest — AUTO-GENERATED by \`pnpm vendor:games\`.
 * Gitignored: the games are fetched locally (scripts/vendor-godot-games.mjs),
 * not committed. fixturesAll.ts merges this in when present. Do not edit.
 */

import type { Fixture } from './fixtures';

export const gameFixtures: Fixture[] = ${JSON.stringify(sortedGameFixtures, null, 2)};
`;
  writeFileSync(gamesOutputPath, gamesOutput);
  console.log(`✅ Generated fixtures.games.ts with ${gameFixtures.length} game fixtures across ${gameCategories.length} categories`);
} else if (existsSync(gamesOutputPath)) {
  rmSync(gamesOutputPath);
  console.log('🧹 Removed stale fixtures.games.ts (no games vendored)');
}
