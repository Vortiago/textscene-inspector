...
The composed runtime tree the user navigates: the **SceneGraph** with **PackedScene instancing** folded in and **GLBSceneRoot** internals descended.
It lives in one node-path space, with a per-sub-scene **ExtResource** scope. It depends on the **resource event bus** caches, so it is derived on demand from a cache snapshot. `liveSceneTree.ts` defines the single traversal. Its consumers are the 3D viewport and 2D world (**NodeDispatcher**), the **Control overlay** (`ControlDispatcher`) and the scene tree panel. The inspector resolver (`useLiveNode`) and the cameras and stats panels consume it too. A consumer that reads **SceneGraph** children directly sees an instance as a childless node and drops everything inside it.
_Avoid_: conflating with **SceneGraph** (parsed, static, root scene only) or "scene tree" (the UI panel).
...
`res://file.tres::SubId`, Godot's own notation for a **SubResource** of some `.tres` other than the previewed scene.
It is the one string that makes all three kinds of resource reference interchangeable to a consumer. The three kinds are an **ExtResource** `.tres`, a SubResource of the scene (resolved through `SceneResourcesContext`), and a SubResource of a `.tres` the scene pulled in. The whole address is the resource identity: the processor cache key, the in-flight dedupe key and `useResource`'s LRU pin. Only real files can be fetched, so the **resource event bus**'s byte layer, a host `ResourceProvider` and **Dependency hot-reload** see only the `filePath` half. A failed address is reported under the address, so it can surface as a missing-resources row. The panel normalises to `filePath` before handing a host a **Resource upload** or its removal. Consumers hold the address as an ordinary path (`ArrayMeshResource.materialPaths`, `MeshLibraryItem.meshPath`, `ExternalMaterialSlot.path`), so no consumer needs to know the third kind exists. A producer minting addresses for a new resource type must honour the id in its processor's `process()`, declared with `addressesSubResources`. Decision: ADR-0029.
_Avoid_: "composite path" or "synthetic path" (it is Godot's grammar). Treating it as a filename (nothing may `fetch` one). A per-consumer resolver for the third kind.
...
The 2D-overlay analogue of NodeComponentRegistry, mapping a Control `typeName` to a DOM component.
It is kept separate so the 3D registry stays THREE-typed.
_Avoid_: "UI registry".
...
**ControlDispatcher**:
The DOM analogue of NodeDispatcher.
It recursively walks a Control subtree and emits nested `<div>`s, mapping anchor, offset and container layout to CSS.
_Avoid_: "UI renderer".

**Viewport mode**:
The single `'2D' | '3D'` display state of the centre viewport.
`3D` mounts the R3F canvas. `2D` mounts the pannable 2D stage: project-viewport frame, 2D world canvas and **Control overlay**, in Godot's 2D-editor order. An auto-default heuristic on the scene root type chooses it. The toolbar toggle overrides it.
_Avoid_: "2D mode" alone (it is one of two values of one state).

**Control overlay**:
The HTML/CSS DOM rendering of a Godot Control or CanvasLayer subtree, layered as a sibling of the R3F `<Canvas>`, never inside it.
_Avoid_: "HUD", "UI canvas".

**Sub-viewport**:
Godot's `SubViewport` Node: a canvas boundary, not a world boundary.
It always owns its World2D, so its CanvasItem descendants (2D world and Control UI) draw nowhere in the parent. It shares the parent's World3D unless `own_world_3d`, so its Node3D descendants draw in the parent's 3D view as through a plain `Node`. It draws nothing itself. Always hyphenated in prose, to keep it distinct from **Viewport mode** (ADR-0030).
...
The `nodePath` to `{ texture, size, readPixels }` lookup a **sub-viewport** publishes its rendered target into.
Every **viewport surface** and `ViewportTexture` consumer resolves against it. It has the same two-context shape as the **AnimationDriverRegistry** (stable register function, reactive map), because it is the same problem: a node publishing something others resolve by NodePath. It carries both a WebGL texture and a CPU-pixel reader, because the DOM overlay cannot sample a WebGL texture. Consumers never learn whether 3D content, 2D-canvas content or a rasterised Control subtree produced it.
...
**layout_mode**:
The Godot Control property recording how a node is positioned: `0` free position, `1` anchors, `2` container-managed.
In this corpus `2` is the majority. The renderer does not branch on this field. It is parsed into `ControlProperties.layoutMode` and left unread. The free-versus-container decision is made structurally from the parent container's imposed `ParentLayoutKind` (`r3f/controls/controlLayout.ts`). A Control is container-managed exactly when its parent is a layout container.
...
**Anchor / offset**:
Godot Control layout properties (`anchors_preset`, `anchor_*`, `offset_*`, `grow_*`), decoded through the full LayoutPreset 0..15 table (`PRESET_ANCHORS`) to CSS absolute positioning.
They apply when the parent imposes the `'free'` layout kind. That is a top-level overlay, or a child of a plain Control, Panel or CanvasLayer rather than a layout container.
_Avoid_: "margin" (reserve for `MarginContainer`, which maps to CSS padding). Gating this on the child's `layout_mode` value (the gate is the parent's `ParentLayoutKind`).

**StyleBox**:
A Godot Control theme resource (`StyleBoxFlat`, `StyleBoxEmpty`) defining background, border and corner radius.
It maps to CSS `background`, `border` and `border-radius` on the Control overlay.
_Avoid_: "style".
...
**Render intent**:
Which of the three declared outcomes (`NodeComponentRegistry.renderIntent`) a node type takes: `draws`, `transform-only` or **pending**.
`draws` is a visible renderer. `transform-only` is an invisible group that positions children and is finished. The badge and the sheet status read the declared intent, never the absence of a registration. "Renders nothing" is an explicit intent, not an unregistered accident. In-viewport text and collision shapes are opt-in toggles (`showLabels`, `showCollisions`) on the viewport-mode **seam** (ADR-0006, ADR-0008). Three invisible roles are not inert. The **AnimationPlayer** draws nothing but drives sibling objects (ADR-0011). A **geometry contributor** draws nothing but supplies its solid to its **CSG root** (ADR-0027). A **sub-viewport** draws nothing but hosts: it scopes its canvas subtree to a **viewport surface** while its 3D subtree stays in the parent world (ADR-0030). All three are roles layered onto `transform-only`, not a fourth outcome.
...
- A **Node** references **ExtResource**s and **SubResource**s by id. The **resource event bus** resolves ExtResources to files. A `.tres` the scene reached may reference its own SubResources. The bus resolves those under a **Sub-resource path**: it fetches the owning file, then builds the named body out of it (ADR-0029).
...
- **Label3D** (3D, billboarded text in-canvas) is a different subsystem from **Label** and **RichTextLabel** (2D DOM text in the **Control overlay**).
...
- An **AnimationTree driver** owns no clips. It evaluates its `tree_root` at the authored `parameters/*` into a blend program. It then drives the **AnimationPlayer** or **GLB animation driver** its `anim_player` resolves to, found through the **AnimationDriverRegistry** (ADR-0019).
- A **sub-viewport** publishes its render target into the **ViewportTextureRegistry**. A **viewport surface** or a `ViewportTexture` consumer resolves it back by node path. The parent's **NodeDispatcher** and **ControlDispatcher** both stop at the boundary. The subtree is therefore dispatched exactly once, by its surface (ADR-0030).
...
> **Dev:** "When `main.tscn` loads, with a Node3D root holding a Hallway plus five CanvasLayer UI scenes, which **viewport mode** do we default to?"
> **Architect:** "3D, because the root is spatial. The five **Control overlay** subtrees do not render in 3D mode, same as Godot's own 3D editor viewport. We surface a 'contains 2D UI' hint so the user can flip the toggle."
...
- "Shape": the CollisionShape3D Node holds a `shape` reference. The **collision-shape resource** carries the geometry.
- "Container" is reserved for Godot's 2D layout Controls (VBoxContainer and the rest). A physics body is a **transform-only group**, never a "transform container".
- "Registry" names three distinct singletons: **NodeRegistry** (parse), **NodeComponentRegistry** (3D render), **ControlComponentRegistry** (2D render). Their multiplicity is the mechanism that keeps the linter React-free, not duplication.
- "id": the **UID reference** is the global `uid://`. `id=` is the per-file resource handle.
- "AnimationClip": the parsed form is a **GodotAnimation**. The runtime object is `THREE.AnimationClip`, always qualified.
- "GLB AnimationPlayer": the tree synthesises a tree-only `GLBAnimationPlayer` row, because Godot exposes glTF clips on an in-hierarchy AnimationPlayer. That row, not the GLB root, activates the **Animation transport** (ADR-0014).
- "Zoom" means two quantities, so always qualify which. In 2D it is a CSS scale factor (0.1 to 4, shown as a percentage in the stage's zoom HUD). In 3D it is the **Editor cursor**'s orbit radius in world units (0.01 to 10000). A wheel zoom anchors to the pointer in both. A touch pinch anchors to the fingers in 2D. In 3D a pinch scales the orbit radius from its seeded distance. "Zoom in" is a smaller number in 3D and a larger one in 2D.
- "Host": **Host (app)** is the embedding application (web previewer or VS Code extension). VS Code's process is always written qualified as "extension host".
- "Viewport": bare "viewport" is the previewer's centre panel (**Viewport mode**, the 2D/3D toggle). The Godot node is always the hyphenated **sub-viewport**. The thing displaying it is a **viewport surface** (ADR-0030).
- "Offscreen": only a sub-viewport's canvas half is hidden from the parent. It owns its World2D but shares the parent's World3D unless `own_world_3d`. Its 3D descendants render in the parent view exactly as through a plain Node. Measured, not derived (ADR-0030).
