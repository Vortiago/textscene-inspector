...
The renderer's grammar is FINITE by choice: `inf` / `-inf` / `inf_neg` / `nan` are legal TSCN literals, but a component that reaches three.js as `Infinity` renders NaN geometry, so the decoders warn-then-fall-back on one exactly as they do on a malformed value. The linter reads Godot's full tokenizer grammar instead, through `TSCN_FLOAT_PATTERN_SOURCE` (`linter/validators/commonValidators.ts`), which is `FLOAT_PATTERN_SOURCE` plus those four spellings.
