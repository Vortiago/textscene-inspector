...
The properties a native Control painter may see: everything its node carries MINUS `modulate` and `selfModulate`. Both fields do exist on the Control — **ControlCanvasWalker** has simply already consumed them (`modulate` folded into the ambient `Modulate2DContext` it wraps the painter in, `self_modulate` read by `useControlOwnTint`), so a painter re-reading either would apply it a second time and square it. `painterView<T>(solveNode)` is a zero-allocation cast of the same object, because painters memoize on props identity; the narrowing is type-level only, and `painterViewConformance.test.ts` is what holds it at runtime by forbidding the spelling anywhere under `nodes/2d/ui` or in a painter outside it (ADR-0037).
