...
Each sheet shows how this previewer renders one Godot node type against real
Godot 4.6.3, from the same camera.
Images are regenerated by `scripts/compare-docs/capture.mjs`. The
gallery is `index.html`, which `pnpm docs:gallery` rebuilds and
`pnpm docs:gallery --check` holds to the sheets in `validate` and CI.
...
verified in CI). It lists the validators and rules that reach that node type.
Hand-written prose below it says what the lenient parser does instead. The four
facts below are true of every node, so this file states them once.
...
substitutes a documented default and continues. Each sheet's prose describes
that substitution.

**The lenient parser swallows five structural errors silently**, with no
...
**Two classes of property are never format-checked, even by strict.**
...
validated. Existence checks stay silent across an instance boundary on purpose.
...
of limitations, so this file records them once.
