...
with a tall dark-rock column filling the right edge. Both images show the same whole level
from real Godot and from this previewer.
...
colours, and both frames draw the coins. Two content differences remain, measured
...
The previewer draws a platform underside and its hanging vines, clipped by the top edge,
across rows 0..8, where Godot's frame is open sky. Godot's lower-right platform runs to
the right frame edge with a full tree crown, and the previewer's stops at x = 759. The
cause of neither difference is identified.
