...
systems controls, a briefing, an action row, and a second column with a sector
map and a status readout. An alert strip lies over the whole frame. Both images
show the same panel from real Godot and from this previewer, framed by the
1152x648 project viewport.

The scene composes 23 Control types, so it moves many variables at once. A
shifted pixel can come from the split solve, the box flow, a stylebox content
margin, a font metric or the canvas snap, and this frame cannot say which. It
catches the interactions between Controls, which a single-variable scene cannot.
Localise and pin a regression seen here in the `unit-*` fixture that owns the
type.
...
- A ScrollContainer that overflows: the clip edge and the vertical
...
- Theme overrides where the composition needs them: the card styleboxes and
  their content margins, the separation and margin constants, and the font size
  and colour overrides. No Theme resource is applied, so every unstyled widget
  resolves through the bundled default theme.

Control itself draws no ink here. The root and the action row's spacer are pure
layout, visible only as the space they hold open.

The scene is deterministic: no animation, no particles, no timers, no random
seed. The picture at settle 0 is its only picture.

## Divergences

The whole frame differs by 11,453 px (1.534 %) in 520 clusters, none bigger than
456 px. Text accounts for all of it. Every card plate, split gutter, clip edge,
scrollbar rect, stylebox colour and `modulate` product lands on the same pixel on
both sides, the composited SubViewport surface and the two chained-tint bars
included. Layout matches too: `Control.get_rect()` per node, printed from Godot
with the scene in a 1152x648 `SubViewport`, gives 55 rects, and the previewer's
solve reproduces each one exactly.

The text difference is glyph rasterisation, the MSDF-atlas residual. A text run
lands within one column of Godot's in either direction, and its rows and wrap
points match exactly. `Apply` inks `x 28..70` in Godot against `x 28..71` here.
The header subtitle inks `x 101..669` against `x 101..668`, and wraps on the same
word at the same two rows. The status column's squad note inks `x 834..1100`
against `x 833..1099`. Most of the difference is in the briefing RichTextLabel at
`y 380..420`, the densest text in the frame.

The `[u]` rule is the one text feature that differs by more than antialiasing. At
`x 500`, Godot draws one crisp row at `y 418` and the previewer draws a two-row
feather at `y 416..417`. The underline sits two pixels high and half a level
soft.

The probes below come from
...
| (24, 80) | the header card's left plate edge, that is, the outer MarginContainer inset | rgb(33, 43, 56) | rgb(33, 43, 56) |
