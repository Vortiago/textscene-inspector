...
- The hint widened per deferred refinement (b): the live-tree predicate `useLiveSceneNodes(isCanvasItemNode)` (UI **or** 2D world nodes, now including instanced sub-scenes) → "Has 2D content — switch to 2D", shown only in the 3D workspace for mixed scenes (Godot has no such affordance; kept as a previewer nicety).
