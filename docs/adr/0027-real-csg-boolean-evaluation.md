...
time and found `unit-csg-cylinder` **0.788%** out, 7.9 times the visual gate, while its
golden was green. Goldens compare us to ourselves, so they detect change and never
wrongness.
...
averages them into one. Every CSG solid is built from Godot's own brush construction
and finished through the shared rule. The geometry a node draws alone and the geometry
it contributes to a boolean are therefore the same triangles.
...
without its builder. That failure mode is a node that renders alone and silently vanishes
inside a boolean.
...
the bug we had. References refreshable only by someone with Godot installed are a
...
- Non-manifold input is not cheaply detectable. `three-bvh-csg` tolerates it silently
  rather than throwing. Godot's `snap` is the first knob to try if artefacts appear.
- `unit-csg-transparency` measures 1.067%, over the gate. Only the BLUE channel is short,
  ...
  reflection on a glossy transparent surface rather than an alpha-blending error. It is
  tracked in #347. The same measurement turned up two larger parity gaps, neither related
  to CSG, tracked there too: an opaque albedo-only sphere reading too blue in shadow, and
  Label3D text rasterisation.

Recorded because a reader finding a CSG library in a *previewer* will reasonably ask
whether it belongs there. The answer to "why not the react-three wrapper" is also not
