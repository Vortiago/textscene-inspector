...
4. **Nothing described the linter.** The repo ships 41 registered rules reporting
   under 211 distinct rule names, across 62 validator-bearing node types; the
   parity corpus said nothing about any of it, and nothing recorded what the
   lenient parser does with a value strict rejects.

## Decision

**A sheet is slice content.** It moves to `<slice>/comparison.md`, beside the
`parser.ts` and `linterParser.ts` it documents. The four `complex-*` whole-scene
...
because listing 31 inherited `Node3D` rows on every light sheet would recreate the
...
- **`AreaLight3D` is absent from the pinned Godot 4.6.3 ClassDB but present in
  current `stable`.** Links are deliberately unpinned (`stable` / `master`), so it
  is carried in the catalog's `extras` array and self-heals into `nodes` when the
  local Godot lists it. Its parity captures cannot have come from the pinned
...
- **`@textscene/core`'s `dist/` is not loadable by plain Node ESM.** Its sources
  ...
  a resolve hook that retries with `.js`. The root fix is a codemod adding
  extensions to source imports, which would delete the shim.
...
factories build theirs by interpolation in another file, so attributing a name to
